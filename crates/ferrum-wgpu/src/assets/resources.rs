...
impl AssetSource for Asset {
    ...
    fn format_url(file_name: &str) -> reqwest::Url {
        let base =
            reqwest::Url::parse("https://pub-72c0c8dd15e249a0a448095fb52cc05c.r2.dev/").unwrap();
    ...
    fn resolve_resource_path(file_name: &str) -> anyhow::Result<PathBuf> {
        ...
        if let Result::Ok(exe_path) = std::env::current_exe()
            && let Option::Some(exe_dir) = exe_path.parent()
        {
            for candidate_name in &candidate_names {
    ...
    async fn load_texture(
        ...
        let data: Vec<u8> = Self::load_binary(file_name).await?;
    ...
    async fn load_model(
        ...
        let instances_raws: Vec<assets::InstanceRaw> =
